...
const inputDir = path.join(root, 'public/images/hero/mizuno');
const outputDir = path.join(root, 'public/hero');
const manifestPath = path.join(root, 'src/data/mizuno/hero-videos.ts');
...
function outputNameFor(studentId) {
  return `hero_mizuno_${studentId}.mp4`;
}
...
function writeManifest(webPaths) {
  const lines = [
    '/** 自動生成: npm run encode:hero-mizuno */',
    'export const MIZUNO_HERO_VIDEOS = [',
    ...webPaths.map((p) => `  '${p}',`),
    '] as const;',
    '',
    'export type MizunoHeroVideoPath = (typeof MIZUNO_HERO_VIDEOS)[number];',
    '',
  ];
